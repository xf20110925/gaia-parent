 ...
 */
public class IndexArticleContent {
    ...
    protected static XContentBuilder convertDocToContentObject(Document doc, int type){
        double relRate = 1.0;
        ...
        XContentBuilder contentBuilderContent = null;
        try {
            contentBuilderContent = JsonXContent.contentBuilder().startObject();
            if (doc.getInteger("readNum") != null){
                contentBuilderContent.field("readNum", doc.getInteger("readNum"));
            }

            if (doc.getInteger("zpzNum") != null){
                contentBuilderContent.field("zpzNum", doc.getInteger("zpzNum"));
            }

            if (doc.getInteger("isOriginal") != null){
                contentBuilderContent.field("isOriginal", doc.getInteger("isOriginal"));
            }

            if (doc.getString("content") != null){
                contentBuilderContent.field("content", doc.getString("content"));
            }else {
...
/*
            if (doc.getString("filePath") != null){
                String content = fileUtils.getFileByPath(doc.getString("filePath"));
                if (content != null && content.length() > 20){
                    String text = Jsoup.parseBodyFragment(content).text();
                    if (text != null){
                        contentBuilderContent.field("content", text);
                    }
                }else {
                    logger.warn("没有content字段或content字段太短! content:{}",content);
                    return null;
                }
            }
*/
            ...
            if (doc.getString("pmid") != null){
                contentBuilderContent.field("pmid", doc.getString("pmid"));
            }

            if (doc.getLong("postTime") != null){
                contentBuilderContent.field("postTime", doc.getLong("postTime"));
            }

            if (doc.getInteger("articleType") != null){
                contentBuilderContent.field("articleType", doc.getInteger("articleType"));
            }

            contentBuilderContent.field("preScore", relRate);

            if (doc.getString("articleUrl") != null){
                contentBuilderContent.field("articleUrl", doc.getString("articleUrl"));
            }

            contentBuilderContent.endObject();
        } catch (IOException e) {
            //e.printStackTrace();
            logger.error("to content error!", e);
        }
        return contentBuilderContent;
    }
    ...
    public void importToEs(int type){
        ...
        while (mongoCursor.hasNext()){
            try {
                ...
                contentBuilder = this.convertDocToContentObject(doc, type);
